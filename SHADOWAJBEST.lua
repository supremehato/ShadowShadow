...
local function validateKey(key, hwid)
    ...
    if not responseBody or responseBody == "" then
        warn("❌ Key Validation Error: Empty response from server (Status: " .. tostring(statusCode) .. ")")
    ...
    if responseBody:match("^%s*<!") or responseBody:match("<html") or responseBody:match("<HTML") then
        warn("❌ Key Validation Error: Server returned HTML instead of JSON (Status: " .. tostring(statusCode) .. ")")
        warn("API URL: " .. KEY_API_URL .. "/validate")
    ...
    if not parseSuccess then
        warn("❌ Key Validation Error: Invalid JSON response (Status: " .. tostring(statusCode) .. ")")
        warn("API URL: " .. KEY_API_URL .. "/validate")
        warn("Response preview: " .. responseBody:sub(1, 200))
        return false, "SERVER_ERROR"
    end
    ...
    elseif statusCode == 404 then
        warn("❌ Key Validation Error: API endpoint not found (404)")
        warn("API URL: " .. KEY_API_URL .. "/validate")
        warn("Make sure the API server is running and the URL is correct!")
        return false, "SERVER_ERROR"
    else
        warn("❌ Key Validation Error: Unexpected status code " .. tostring(statusCode))
        warn("API URL: " .. KEY_API_URL .. "/validate")
        return false, "SERVER_ERROR"
    end
end
...
if reason == "HWID_BOUND" then
    print("✅ Key validated! This device has been bound to your key.")
else
    print("✅ Key validated! Welcome back.")
end
...
local function markClientAsLoaded()
    ...
    if not success then
        warn("[Client] ⚠️ Failed to mark client as loaded:", response)
...
task.wait(0.5)
print("[Client] ✅ Client load time recorded:", clientLoadedTime, "- Safe to connect WebSocket/check jobs")
                ...
                local success, result = pcall(function()
                    ...
                    if data and data.client_ids then
                        ...
                        if hasChanges then
                            ...
                            print(string.format("🔄 ESP: Updated %d tracked client IDs", #trackedClientIds))
...
local function initializeESP()
    ...
    print("👁️ ESP System Initialized (Real-Time Mode)")
    print(" 🔴 Red = Bot Users (with 'Aether Elite user' label)")
    print(" 🟢 Dark Green = Normal Players")
end
...
getEliteSettings()
print("[Aether Elite] ⚙️ Settings loaded - MinValue:", Settings.MinValue / 1000000, "M/s")
...
task.spawn(function()
    ...
    if petList and #petList > 0 then
        ...
        print("[Aether Elite] ✅ Initialized " .. #petList .. " pets in PriorityPets and BlacklistedPets")
    end
end)
...
task.spawn(function()
    ...
    if not petList or #petList == 0 then
        warn("[Aether Elite] ⚠️ No pets found, retrying...")
        task.wait(2)
        petList = getPetList()
    end
    ...
    if petList and #petList > 0 then
        ...
        print("[Aether Elite] 📋 Populating Ignore List tab with", ignorePetCount, "pets...")
        ...
        print("[Aether Elite] ✅ Ignore List tab populated")
        ...
        print("[Aether Elite] ✅ Priority List tab populated")
    end
end)
...
local function populateIgnoreListTab(scrollingFrame)
    ...
    print("[Aether Elite] 📋 Adding", #petList, "pets to Ignore List tab...")
    for i, petName in ipairs(petList) do
        local isSelected = Settings.BlacklistedPets[petName] == true
        pcall(function()
    ...
    print("[Aether Elite] ✅ Ignore List tab populated with", #petList, "pets")
end
...
local function populatePriorityListTab(scrollingFrame)
    ...
    print("[Aether Elite] 📋 Adding", #petList, "pets to Priority List tab...")
    for i, petName in ipairs(petList) do
        local isSelected = Settings.PriorityPets[petName] == true
        pcall(function()
    ...
    print("[Aether Elite] ✅ Priority List tab populated with", #petList, "pets")
end
...
startAjoButton = prioTab:NewButton("▶ START", function()
    ...
    for i = 1, maxRetries do
        if retryCountInput then
            ...
            if type(retryCountInput) == "table" and retryCountInput.Text then
                ...
                print("[Aether Elite] ✅ Retries loaded:", Settings.Retries)
                break
    ...
    for i = 1, maxRetries do
        if retryTimeInput then
            ...
            if type(retryTimeInput) == "table" and retryTimeInput.Text then
                ...
                print("[Aether Elite] ✅ RetryDelay loaded:", Settings.RetryDelay, "s")
                break
...
local saveSettingsButton = prioTab:NewButton("💾 Save Settings", function()
    pcall(saveSettingsFromUI)
...
local function ensureLogsContentReady()
    ...
    if not listLayout or not listLayout.Parent then
        ...
        if not success then
            warn("[Aether Elite] ⚠️ Failed to set listLayout parent")
            return false
        end
    end
...
task.spawn(function()
    ...
    local function setTitleBlue()
        if scriptedUI then
            ...
            if titleLabel then
                ...
                print("[Aether Elite] ✅ Title color set to blue and locked")
            else
    ...
    if prioTab and prioTab.sectionBG then
        print("[Aether Elite] ✅ PRIO tab created")
    end
    if logsTab and logsTab.sectionBG then
        print("[Aether Elite] ✅ LOGS tab created")
    end
    if logsContent then
        print("[Aether Elite] ✅ LOGS content ScrollingFrame ready")
    end
    if logsContent then
        logsContent.Visible = true
        logsContent.ZIndex = 11
        print("[Aether Elite] ✅ LOGS content area created and visible")
    end
    -- mainContainer is a ScreenGui, doesn't have Size property
    if mainContainer then
        print("[Aether Elite] ✅ Main container (ScreenGui) initialized")
    end
    print("[Aether Elite] ✅ PRIO container position:", prioContainer and prioContainer.Position.X.Scale or "nil")
    print("[Aether Elite] ✅ LOGS container position:", logsContainer and logsContainer.Position.X.Scale or "nil")
...
task.spawn(function()
    ...
    for i = 1, maxRetries do
        ...
        if verified then
            print("[Aether Elite] ✅ Settings loaded into UI - MinValue:", Settings.MinValue / 1000000, "M/s | Retries:", Settings.Retries, "| RetryDelay:", Settings.RetryDelay, "s")
            break
        end
...
local function toggleIgnoreListPopup()
    ...
    placeholderInput.PlaceholderText = "🔍 Search pets to ignore..."
...
local function togglePriorityListPopup()
    ...
    placeholderInput.BorderSizePixel = 0
    placeholderInput.Text = ""
    placeholderInput.PlaceholderText = "✨ Search priority pets..."
    placeholderInput.PlaceholderColor3 = Color3.fromRGB(120, 180, 220) -- Blue tinted gray
    placeholderInput.TextColor3 = Color3.fromRGB(200, 240, 255) -- Very Light Blue
    placeholderInput.TextSize = 15
    placeholderInput.Font = Enum.Font.GothamBold
...
local function togglePopupWindow(popupName, title, isPriorityList)
    ...
    placeholderInput.BorderSizePixel = 0
    placeholderInput.Text = ""
    placeholderInput.PlaceholderText = "🔎 Search pets..."
...
local function joinServerWithRetries(serverId, gui, allowRejoin)
    ...
    if currentJobId == targetServerId then
        if not allowRejoin then
            print("[Join] ⚠️ Already in server " .. targetServerId .. ", skipping join")
            return false
        else
            print("[Join] ℹ️ Already in server " .. targetServerId .. ", but allowing rejoin (manual click)")
        end
    end
    ...
    if tostring(game.JobId) == targetServerId then
        print("[Join] ✅ Successfully joined server " .. targetServerId)
...
local function addLog(petName, petValue, serverId, thumbnailUrl, gui, petValueFormatted, isPriority)
    ...
    if not logsTab then
        warn("[Aether Elite] ⚠️ logsTab not available - log will not be displayed")
        return
    end
    
    -- Check if logsTab has NewButton method
    if not logsTab.NewButton or type(logsTab.NewButton) ~= "function" then
        warn("[Aether Elite] ⚠️ logsTab.NewButton not available - log will not be displayed")
        return
    end
    ...
    print("[Aether Elite] 📝 Adding log:", petName, "Value:", petValueFormatted or formatPetValue(petValue), "Server:", serverId)
    ...
    if logButtons and logButtons[serverId] then
        print("[Aether Elite] ℹ️ Log button for server", serverId, "already exists, skipping duplicate.")
        return
    end
    
    if logDataStore and logDataStore[serverId] then
        print("[Aether Elite] ℹ️ Log data for server", serverId, "already exists, skipping duplicate.")
        return
    end
    ...
    if not logsTab or not logsTab.NewButton then
        warn("[Aether Elite] ⚠️ logsTab not ready - waiting and retrying...")
        task.wait(0.1)
        if not logsTab or not logsTab.NewButton then
            warn("[Aether Elite] ⚠️ logsTab still not ready - log will not be displayed")
            return
        end
    end
    ...
    local logButton = logsTab:NewButton(logText, function()
        print("[Aether Elite] 🖱️ Log button clicked for:", petName, "Server:", serverId)
    ...
    task.spawn(function()
        ...
        while true do
            ...
            local success, err = pcall(function()
                ...
                print("[WebSocket] ✅ Connected successfully - Stable connection established")
                ...
                local messageHandler = function(message)
                    ...
                    if type(message) == "string" and string.match(message, "^%d+") then
                        ...
                        elseif msgType == 1 then
                            ...
                            print("[WebSocket] ℹ️ Received close packet (normal protocol message)")
                            processed = true
                            ...
                            if eventData then
                                ...
                                if decodeSuccess and type(eventArray) == "table" and #eventArray >= 2 then
                                    ...
                                    if eventName == "registered" then
                                        print("[WebSocket] ✅ Registered successfully - Connection stable")
                                        processed = true
                                    elseif eventName == "job" then
                                        ...
                                        if data and data.has_job then
                                            ...
                                            if clientLoadedTime == 0 or not clientLoadedSuccess then
                                                print("[WebSocket] ⚠️ Rejecting job - client load time not confirmed yet")
                                                processed = true
                                                ...
                                                if jobCreatedTime > 0 and clientLoadedTime > 0 then
                                                    ...
                                                    if jobCreatedTime < clientLoadedTime then
                                                        print("[WebSocket] ⚠️ REJECTING OLD JOB - Created:", jobCreatedTime, "Client loaded:", clientLoadedTime, "Diff:", (clientLoadedTime - jobCreatedTime), "seconds")
                                                        -- Skip this job - don't add to pendingJobs
                                                        processed = true
                                                    else
                                                        -- Job is newer than client load time - accept it
                                                        print("[WebSocket] ✅ ACCEPTING JOB - Created:", jobCreatedTime, "Client loaded:", clientLoadedTime, "Diff:", (jobCreatedTime - clientLoadedTime), "seconds")
                                                        ...
                                                        print("[WebSocket] ✅ Job queued:", data.pet_name or "Unknown")
                                                    end
                                                else
                                                    -- Invalid timestamps - reject to be safe
                                                    warn("[WebSocket] ⚠️ Rejecting job - invalid timestamps (job:", jobCreatedTime, "client:", clientLoadedTime, ")")
                                                    processed = true
                                                end
                                            end
                                        end
                        ...
                        elseif msgType == 41 then
                            ...
                            print("[WebSocket] ℹ️ Received disconnect packet (normal protocol message)")
                            processed = true
                        end
                    end
                        ...
                        if decodeSuccess and data then
                            ...
                            elseif data.type == "job" then
                                ...
                                if data.has_job then
                                    -- CRITICAL: Don't process ANY jobs until client load time is confirmed
                                    if clientLoadedTime == 0 or not clientLoadedSuccess then
                                        print("[WebSocket] ⚠️ Rejecting job (JSON) - client load time not confirmed yet")
                                        -- Skip this job completely
                                    else
                                        -- CRITICAL: Filter out old jobs created before client loaded
                                        local jobCreatedTime = tonumber(data.created_time) or 0
                                        ...
                                        if jobCreatedTime > 0 and clientLoadedTime > 0 then
                                            -- Skip if job was created BEFORE client loaded
                                            if jobCreatedTime < clientLoadedTime then
                                                print("[WebSocket] ⚠️ REJECTING OLD JOB (JSON) - Created:", jobCreatedTime, "Client loaded:", clientLoadedTime, "Diff:", (clientLoadedTime - jobCreatedTime), "seconds")
                                                -- Skip this job
                                            else
                                                -- Job is newer than client load time - accept it
                                                print("[WebSocket] ✅ ACCEPTING JOB (JSON) - Created:", jobCreatedTime, "Client loaded:", clientLoadedTime, "Diff:", (jobCreatedTime - clientLoadedTime), "seconds")
                                                ...
                                                -- Add to pending jobs queue (only if not old)
                                                table.insert(pendingJobs, data)
                                                print("[WebSocket] ✅ Job queued (JSON):", data.pet_name or "Unknown")
                                            end
                                        else
                                            -- Invalid timestamps - reject to be safe
                                            warn("[WebSocket] ⚠️ Rejecting job (JSON) - invalid timestamps (job:", jobCreatedTime, "client:", clientLoadedTime, ")")
                                        end
                                    end
                                end
                ...
                print("[WebSocket] ✅ Connection fully established - PERMANENT CONNECTION")
                print("[WebSocket] ℹ️ Close handler disabled - Connection is persistent")
                print("[WebSocket] ℹ️ Connection will NEVER auto-disconnect")
                ...
                print("[WebSocket] ℹ️ Error handler disabled - connection is fault-tolerant")
                        ...
                        if wsConnection and wsConnected then
                            ...
                            if not success then
                                ...
                                if failedPings % 10 == 0 then
                                    print("[WebSocket] ℹ️ " .. failedPings .. " pings failed, but connection persists")
                                end
                            else
                                if failedPings > 0 then
                                    print("[WebSocket] ✅ Ping recovered after " .. failedPings .. " failures")
                                end
...
task.spawn(function()
    ...
    if clientLoadedTime == 0 or not clientLoadedSuccess then
        warn("[WebSocket] ⚠️ Client load time not set properly, waiting...")
        ...
        if clientLoadedTime == 0 then
            warn("[WebSocket] ⚠️ Client load time still not set, connecting anyway (may receive old jobs)")
        else
            print("[WebSocket] ✅ Client load time confirmed:", clientLoadedTime, "- Safe to connect")
        end
    else
        print("[WebSocket] ✅ Client load time confirmed:", clientLoadedTime, "- Safe to connect")
    end
    
    print("[WebSocket] 🔌 Initiating persistent WebSocket connection...")
    connectWebSocket()
end)
...
task.spawn(function()
    while true do
        ...
        if not wsConnected and not isConnecting then
            print("[WebSocket Monitor] 🔄 External disconnection detected, reconnecting...")
...
local function checkForJob()
    ...
    if clientLoadedTime == 0 or not clientLoadedSuccess then
        ...
        print("[Job Check] ⚠️ Skipping job check - client load time not confirmed")
    ...
    if #pendingJobs > 0 then
        ...
        for i, job in ipairs(pendingJobs) do
            ...
            if jobCreatedTime > 0 and clientLoadedTime > 0 then
                ...
                if jobCreatedTime >= clientLoadedTime then
                    ...
                    print("[Job Filter] ⚠️ Removing old job from queue (created:", jobCreatedTime, "client loaded:", clientLoadedTime, "diff:", (clientLoadedTime - jobCreatedTime), "seconds)")
                end
            else
                -- Invalid timestamp - reject to be safe
                removedCount = removedCount + 1
                warn("[Job Filter] ⚠️ Removing job with invalid timestamp")
            end
        end
        
        -- Replace global pendingJobs queue with filtered jobs
        if removedCount > 0 then
            print("[Job Filter] 🧹 Removed", removedCount, "old/invalid jobs from queue")
        end
        ...
        if #pendingJobs > 0 then
            ...
            print("[Job Filter] ✅ Returning job (created:", job.created_time, "client loaded:", clientLoadedTime, ")")
            return job
        end
    end
        ...
        while true do
            ...
            for quickCheck = 1, maxQuickChecks do
                ...
                if jobData.has_job and jobData.server_id then
                    ...
                    if isPetBlacklisted(petName) then
                        print(string.format("[Job Rejected] 🚫 Pet %s is BLACKLISTED (rejecting regardless of priority)", petName))
                        ...
                        if isPriority then
                            ...
                            print(string.format("[Job Accepted] ⭐ PRIORITY PET: %s", petName))
            ...
            if #priorityJobs > 0 then
                ...
                for _, jobData in ipairs(priorityJobs) do
                    local petValue = tonumber(jobData.pet_value) or 0
                    local petName = jobData.pet_name or "Unknown"
                            print(string.format("[Job Accepted] ⭐ PRIORITY PET: %s (Value: %.0f)", petName, petValue))
                    ...
                    -- If currently joining a normal pet, stop and join priority instead
                        ...
                        print("[Auto-Join] ⚠️ Priority pet detected! Canceling current join for priority")
                                    stopJoining = true
                    ...
                    print(string.format("[Auto-Join] 🚀 Starting join for priority pet %s (from %d priority pets)", priorityPetName, #priorityJobs))
                    ...
                    for i = 2, #priorityJobs do
                        ...
                        print(string.format("[Auto-Join] ⚠️ Skipping other priority pet %s (already joining first priority)", otherPetName))
            ...
            if #priorityJobs == 0 then
                for _, jobData in ipairs(normalJobs) do
                    ...
                    local petName = jobData.pet_name or "Unknown"
                            ...
                            print(string.format("[Job Accepted] ✅ Pet %s (Value: %.0f)", petName, petValue))
                        ...
                        -- Only join if not already joining
                                if not currentlyJoining then
                                    print(string.format("[Auto-Join] 🚀 Starting join for pet %s", petName))
                            ...
                            print(string.format("[Auto-Join] ⚠️ Already joining, skipping %s", petName))
                ...
                for _, jobData in ipairs(normalJobs) do
                    local petName = jobData.pet_name or "Unknown"
                    print(string.format("[Auto-Join] ⚠️ Priority pets found, skipping normal pet %s", petName))
                        clearJob(jobData.server_id)
                end
            end
